    ...
    >
      {!reduced && (
        <img
          ...
          className={`absolute inset-0 h-full w-full object-cover transform-gpu transition-opacity duration-300 ease-out ${isEnding ? 'opacity-0 scale-105 brightness-125' : 'opacity-100 scale-100 brightness-100'
            }`}
        />
      )}
      ...
      <div
        className={`absolute inset-0 pointer-events-none transition-opacity duration-700 ease-out ${isEnding ? 'opacity-100' : 'opacity-0'
          }`}
      >
