...
| Option | Required | Meaning |
| --- | --- | --- |
| `word` | Yes | The answer players will guess. One or two words, up to 32 characters per word. |
...
## Automatic NRT rewards

Guessing-game wins and post reactions feed the existing `/nrtleaderboard` balance system automatically:

| Event | Reward | Once-only source |
| --- | ---: | --- |
...
```bash
npm run test:nrt-store
npm run test:nrt-rewards
```
